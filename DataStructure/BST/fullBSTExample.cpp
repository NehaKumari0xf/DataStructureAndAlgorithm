...
void traverseBSTInPreOrder(BSTNode* root)
{
	if(root==NULL)
	return;
	printf("%d, ",root->data);
	traverseBSTInPreOrder(root->left);
	traverseBSTInPreOrder(root->right);
}
void traverseBSTInPostOrder(BSTNode *root)
{
	if(root==NULL)return;
	traverseBSTInPostOrder(root->left);
	traverseBSTInPostOrder(root->right);
	printf("%d, ",root->data);
}
int main()
{
