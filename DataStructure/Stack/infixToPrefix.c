...
int main()
{
    ...
    printf("\nReverse infix expression:%s",infix);
    ...
    for(i=0;infix[i];i++)
    {
        ...
        if(isoperator(scannedSymbol))
        {
            ...
            {       
                ...
                {
                    //pop an operator from stack and append it to prefix
                    pop(stack,&tos,&poppedOperator);
                    append(prefix,poppedOperator);
                    append(prefix,',');
    ...
    append(prefix,',');
    //pop all operator from stack and append it to prefix
    while(tos!=-1)
    {
        pop(stack,&tos,&poppedOperator);
        append(prefix,poppedOperator);
    ...
    do
    {
    	...
    	commaPos=strstr(prefix+firstCommaPosition+1,",");
    	if(commaPos==0)
    	{
    		break;
		}
		secondCommaPosition=commaPos-prefix;
		totalCharacters=secondCommaPosition-firstCommaPosition-1;
		
		//extract data between 
		strncpy(substr,prefix+firstCommaPosition+1,totalCharacters);
